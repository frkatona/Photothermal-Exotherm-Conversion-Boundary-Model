...
$$
u^{n+1} = u_n + \Delta t \, f(u_{n+1}, t_{n+1})
$$
...
$$
\Delta t \le C \frac{\Delta x^2}{\alpha}
$$
...
$$
\alpha^{n+1} = \alpha^n + \Delta t \, R(T^n, \alpha^n)
$$

and then

$$
\left(M \rho c_p + \Delta t \, K\right) T^{n+1}
= M \rho c_p \, T^n + \Delta t \, F(\ldots)
$$
...
- explicit conversion updates can overshoot if $\Delta t$ is too large, which is why the code clamps $\alpha$ into $[0, 1]$
